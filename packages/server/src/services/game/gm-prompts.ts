...
export function buildSetupPrompt(ctx: SetupPromptContext = {}): string {
  ...
  return [
    ...
    `For startingNpcs descriptions, include portrait-visible cues before personality: apparent age/species, build, hair/eyes/skin, clothing, gear/weapon, expression, or silhouette. If a secret/goal is included, keep at least one visible cue in the same sentence so NPC portrait generation has usable identity.`,
    ...
    `      "description": "One sentence: portrait-visible first impression (age/species, build, hair/eyes/skin, clothing, gear/weapon, expression or silhouette), then voice/cadence, desire, and one secret or complication if useful",`,
    `      "location": "region_1",`,
    `      "reputation": 0`,
